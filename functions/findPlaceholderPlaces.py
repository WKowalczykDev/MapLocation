    ...
    for cp in centerPoints:
        ...
        for fp in fixedPoints:
            ...
            if abs(d.x) < w_placeholder and abs(d.y) < h_placeholder:
                ...
                if abs(d.x) < w_placeholder:
                    print("szerokosc")
                    final_px = cp.x+(w_placeholder-abs(d.x)+1)*sign(d.x)
                    fixedPoints.append(Point(final_px, y_from_x(cp,fp,final_px)))
                # przypadek na wysokosc
                elif abs(d.y) < h_placeholder:
                    print("wysokosc")
                    final_py = cp.y + (h_placeholder - abs(d.y)+1) * sign(d.y)
...
# import json
# with open("./../helpers/centerpoints.json", "r", encoding="utf-8") as f:
#     miasta = json.load(f)
#
